...
export function ProductCard({
  ...
  return (
    <motion.article
      ...
      <div className='hidden md:block absolute inset-0 bg-gradient-to-br from-[var(--primary)]/[0.03] via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-700 pointer-events-none' />

      <div className='relative aspect-square overflow-hidden bg-gradient-to-br from-gray-50 via-white to-gray-50/50 p-4 sm:p-6 md:p-8'>
        <Link
          ...
          {image ? (
            <div className="relative w-full h-full">
              <Image
                ...
                alt={name}
                fill
                className='w-full h-full object-contain md:group-hover:scale-110 transition-transform duration-500 md:duration-700 ease-out'
        ...
        <div className='absolute top-2 left-2 md:top-4 md:left-4 right-2 md:right-4 flex items-start justify-end gap-2 pointer-events-none'>
          {!IS_CATALOG_MODE && productDetails.hasDiscount && (
            <motion.div
              initial={{ scale: 0, opacity: 0 }}
              animate={{ scale: 1, opacity: 1 }}
              transition={{ delay: 0.1, type: 'spring', stiffness: 200 }}
              className='bg-gradient-to-br from-red-500 to-red-600 text-white px-2 py-1 md:px-3 md:py-1.5 rounded-lg md:rounded-xl shadow-md md:shadow-lg'>
        ...
        {onQuickView && (
          <motion.button
            ...
            className='hidden md:flex absolute bottom-4 right-4 w-10 h-10 lg:w-12 lg:h-12 bg-white/90 backdrop-blur-sm rounded-full items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-300 shadow-xl border border-gray-200 z-10 cursor-pointer text-[var(--primary)] hover:bg-[var(--primary)] hover:text-white'
      ...
      <div className='relative flex-1 flex flex-col p-4 sm:p-5 md:p-6 gap-3 md:gap-4'>
        <Link
          href={productDetails.url}
          ...
          <h3 className='text-gray-900 font-bold text-sm sm:text-base md:text-lg leading-snug line-clamp-2 min-h-[2.75rem] sm:min-h-[3rem] md:min-h-[3.5rem] md:group-hover/title:text-[var(--primary)] transition-colors duration-300'>
            {name}
          </h3>
        </Link>

        {productDetails.code && (
          <div className='inline-flex items-center gap-1.5 md:gap-2 bg-gray-100 px-2 py-1 md:px-3 md:py-1.5 rounded-md md:rounded-lg self-start border border-gray-200'>
            <Tag className='w-3 h-3 md:w-3.5 md:h-3.5 text-gray-500' />
            <span className='text-[10px] sm:text-xs text-gray-700 font-mono font-bold tracking-wide break-all line-clamp-1 max-w-[150px] sm:max-w-[200px]'>
              {productDetails.code}
            </span>
          </div>
        )}

        <div className='space-y-1.5 md:space-y-2 mt-auto md:mt-2'>
          {!IS_CATALOG_MODE ? (
            <>
              {productDetails.hasDiscount && (
                <div className='flex items-center gap-2 flex-wrap'>
                  <span className='text-xs sm:text-sm text-gray-400 line-through'>
                    R$ {productDetails.finalOriginalPrice.toFixed(2).replace('.', ',')}
                  </span>
              ...
              <div className='flex items-baseline gap-1 md:gap-1.5'>
                <span className='text-xs sm:text-sm text-gray-500 font-semibold'>R$</span>
            ...
            <div className='flex flex-col gap-1 py-1'>
              <span className='text-[10px] sm:text-xs md:text-sm font-bold text-gray-500 uppercase tracking-wide'>
                Preço sob consulta
              </span>
              <div className='h-0.5 w-8 md:w-12 bg-gray-200 mt-0.5 md:mt-1 rounded-full'></div>
            </div>
          )}
        </div>
          ...
          <div
            ...
            <span className='text-[10px] sm:text-xs font-bold leading-tight line-clamp-1'>
              ...
              {estimatedDays && deliveryType !== 'pronta-entrega' && (
                <span className='opacity-70 ml-1 font-normal'>• {estimatedDays}</span>
              )}
            </span>
          </div>
        )}

        <div className={`flex flex-col gap-2 md:gap-2.5 ${!IS_CATALOG_MODE ? 'pt-1 md:pt-2' : ''}`}>
          ...
          <motion.button
            ...
            className={`w-full ${IS_CATALOG_MODE ? 'bg-gradient-to-r from-[#25D366] to-[#128C7E]' : 'bg-[var(--primary)] hover:bg-[#1a2d5f]'} text-white py-2.5 sm:py-3 md:py-4 rounded-lg md:rounded-xl font-bold text-xs sm:text-sm shadow-md md:shadow-lg transition-all flex items-center justify-center gap-1.5 md:gap-2`}
            aria-label={
              IS_CATALOG_MODE
                ? 'Solicitar cotação'
