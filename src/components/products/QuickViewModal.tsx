...
export const QuickViewModal = ({ product, onClose }: QuickViewModalProps) => {
  ...
  const deliveryType =
    (product.deliveryType as keyof typeof DELIVERY_CONFIG) || 'pronta-entrega'
  const config =
    DELIVERY_CONFIG[deliveryType] || DELIVERY_CONFIG['pronta-entrega']
  ...
  const deliveryTitle = IS_CATALOG_MODE
    ? deliveryType === 'pronta-entrega'
      ? 'Entrega Facilitada'
      : deliveryBadge.title
    : deliveryBadge.title
  const deliverySubtitle = IS_CATALOG_MODE
    ? deliveryType === 'pronta-entrega'
      ? 'Consulte regiões'
      : 'Consulte prazos'
    : deliveryBadge.subtitle
                ...
                <div className='grid grid-cols-3 gap-2 mt-4'>
                  <div className='text-center p-3 bg-[var(--neutral-50)] rounded-lg'>
                    <div className='flex items-center justify-center mb-1'>
                      <deliveryBadge.Icon
                        className={`w-6 h-6 ${deliveryBadge.style}`}
                      />
                    </div>
                    <div className='text-[10px] text-[var(--neutral-600)] font-semibold'>
                      {deliveryTitle}
                    </div>
                    <div className='text-[8px] text-[var(--neutral-600)] font-semibold'>
                      {deliverySubtitle}
                    </div>
                  </div>
                  <div className='text-center p-3 bg-[var(--neutral-50)] rounded-lg'>
                    <div className='flex items-center justify-center mb-1'>
                      <warrantyBadge.Icon
                        className={`w-6 h-6 ${warrantyBadge.style}`}
                      />
                    </div>
              ...
              <div className='flex flex-col gap-4'>
                <div>
                  ...
                  <div className='flex items-center gap-2.5 flex-wrap mb-4'>
                    ...
                    {!IS_CATALOG_MODE && (
                      <div
                        className={`${config.style.bg} border-2 ${config.style.border} rounded-xl px-3 py-2 shadow-md inline-flex items-center gap-2`}>
                        <div className='flex items-center gap-1.5'>
                          <div
                            className={`w-2 h-2 rounded-full ${config.style.dot}`}
                          />
                          <DeliveryIcon
                            className={`w-3.5 h-3.5 ${config.style.icon}`}
                          />
                        </div>
                        <span
                          className={`text-xs font-black ${config.style.text}`}>
                          {config.label}
                        </span>
                      </div>
                    )}
                  ...
                  {product.description && (
                    <p className='text-[var(--neutral-700)] leading-relaxed mb-4 text-sm'>
                      {product.name} para máquinas pesadas. Ideal para reposição
                      e manutenção, oferecendo qualidade e confiabilidade para
                      seu equipamento.
                    </p>
                  )}
                </div>
                    ...
                    {!IS_CATALOG_MODE ? (
                      <>
                        <div className='flex items-center gap-2 bg-white border-2 border-[var(--neutral-200)] rounded-xl p-1'>
                          ...
                          <span className='w-12 text-center font-black text-[var(--neutral-900)]'>
                            {quantity}
                          </span>
                          <motion.button
                            whileTap={{ scale: 0.9 }}
